...
def loading_labware(protocol, experiment_dict, well_order='row'):
    ...
    if 'OT2 Modules' in experiment_dict.keys():
        ...
        if modules[0] == 'thermocycler':
            ...
            modules_objects = modules_objects.load_adapter("opentrons_aluminum_flat_bottom_plate")
    ...
    if 'OT2 Modules' in experiment_dict.keys():
        if modules[0] == 'thermocycler':
            dest_labware_objects = modules_objects.load_labware(name="nest_96_wellplate_100ul_pcr_full_skirt")
            dest_wells = module_to_wells(dest_labware_objects)
            dest_labware_slots = ['7']
            dest_labware_names = ["nest_96_wellplate_100ul_pcr_full_skirt"]
    ...
    if 'OT2 Modules' in experiment_dict.keys():
        loaded_labware_dict['Module'] = modules_objects        
...
def pipette_action(protocol, pipette, stock_volume_to_pull, stock_position_to_pull, destination_well, stock_name, resevoir):
    ...
    if stock_volume_to_pull < 20:
        ...
        pass
        #pipette.air_gap(20)
    if stock_name == '2mM_PVP-stock':
        protocol.delay(seconds = 13)
        pipette.touch_tip(v_offset = -8, radius = 0.5) 
    pipette.dispense(stock_volume_to_pull, destination_well.top(2))
    pipette.blow_out()
...
def pipette_volumes_component_wise(protocol, directions, loaded_labware_dict, stock_to_pipette_order=None): # need to add kwargs for the transfer function   
    ...
    #if 'Module' in loaded_labware_dict.keys():
    #    module = loaded_labware_dict['Module'] #Temperature module
    #    module.set_temperature(50)
    ...
    for stock_name in stock_to_pipette_order: #THIS LOOP ITERATES OVER THE STOCK SOLUTOINS
        ...
        for stock_index, stock_instructions in directions.items():
            ...
            stock_position_to_pull = single_stock_instructions['Stock Position']
            destination_well = single_stock_instructions['Destination Well Position']
            resevoir = loaded_labware_dict['Resevoir Wells']
            ...
            if stock_volume_to_pull == 0:
                ...
                pipette_action(protocol, pipette, stock_volume_to_pull, stock_position_to_pull, destination_well, stock_name, resevoir)
            ...
            if stock_name == '0.5M_NaCl-stock':
                pipette.mix(2, 20, destination_well)
            else:
                pass
#                 if stock_volume_to_pull < large_pipette.min_volume:
#                     pipette.mix(2, 20, destination_well)
#                     pipette.mix(2, 20, resevoir[-1], rate=2)
#                     pipette.blow_out()
#                     pipette.mix(2, 20, resevoir[-2], rate=2)
#                     pipette.blow_out()
#                     pipette.mix(2, 20, resevoir[-3], rate=2)
#                     pipette.blow_out()
#                 else:
#                     large_pipette.mix(2, 100, destination_well)
#                     large_pipette.mix(2, 100, resevoir[-1])
#                     large_pipette.blow_out()
#                     large_pipette.mix(2, 100, resevoir[-2])
#                     large_pipette.blow_out()
#                     large_pipette.mix(2, 100, resevoir[-3])
#                     large_pipette.blow_out()
                
#                 if stock_name == '0.025mM_Seeds-stock' and stock_volume_to_pull < large_pipette.min_volume:
#                     large_pipette.mix(2, 100, destination_well)
#                     large_pipette.mix(2, 100, resevoir[-1])
#                     large_pipette.blow_out()
#                     large_pipette.mix(2, 100, resevoir[-2])
...
#                 #Change tip once in a while when pipetting tannic acid
#                 if stock_name == '2mM_TA-stock' and str(destination_well) == 'B12 of Corning 96 Well Plate 360 µL Flat on 6':
#                     large_pipette.drop_tip()
#                     large_pipette.pick_up_tip()
#                 if stock_name == '2mM_TA-stock' and str(destination_well) == 'F12 of Corning 96 Well Plate 360 µL Flat on 6':
#                     large_pipette.drop_tip()
#                     large_pipette.pick_up_tip()
            #else: 
        ...
        #MIXING STEP
        if stock_name == '4xdiluted_Seejjjjjjjds-stock': # DEFINE WHEN TO MIX: THE MIXING WILL OCCUR AFTER: if stock_name == 'stock-stock':
            large_pipette.pick_up_tip()
            for stock_index, stock_instructions in directions.items():
                single_stock_instructions = stock_instructions[stock_name]
                stock_volume_to_pull = single_stock_instructions['Stock Volume']
                ...
                destination_well = single_stock_instructions['Destination Well Position']
                pipette = large_pipette
                pipette.mix(3, 150, destination_well)
                pipette.mix(3, 250, loaded_labware_dict['Resevoir Wells'][-1])
                pipette.mix(2, 250, loaded_labware_dict['Resevoir Wells'][-2])
                pipette.blow_out()
            large_pipette.drop_tip()
    for line in protocol.commands(): 
        print(line)  
